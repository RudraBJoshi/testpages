                            ...
                            <div style="margin-top:8px; padding-top:8px; border-top:1px solid rgba(255,255,255,0.1);">
                                ...
                                <div style="margin-top:12px; padding-top:8px; border-top:1px solid rgba(255,255,255,0.1);">
                                    ...
                                    <div style="margin-top:10px; display:flex; align-items:center; gap:6px;">
                                        <input type="checkbox" id="player-hitbox-rigidbody">
                                        <label for="player-hitbox-rigidbody" style="margin:0;">Use Rigid Body Hitbox</label>
                                    </div>
                                    <div style="margin-top:4px; font-size:0.75em;">
                                        Opt-in: once the sprite loads, the collision box is computed automatically from the sprite's actual pixels (transparency + background color) instead of the manual reduction values above.
                                    </div>
                                </div>
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ui = {
        ...
        pHitboxRigidBody: document.getElementById('player-hitbox-rigidbody'),
    ...
    function updateStepUI() {
        ...
        if (current === 'background') {
            ...
            [ui.pDownRow, ui.pRightRow, ui.pLeftRow, ui.pUpRow, ui.pUpRightRow, ui.pDownRightRow, ui.pUpLeftRow, ui.pDownLeftRow, ui.pDirCols, ui.pHitboxW, ui.pHitboxH, ui.pHitboxRigidBody]
...
function player_extract(ui, p) {
    ...
    return {
     ...
     rigidBody: !!(ui.pHitboxRigidBody && ui.pHitboxRigidBody.checked),
     keypress: keypress
    }
}
...
function player_code(px, p, name = "playerData" ) {
    ...
    const tpl = window.GameTemplatesV1_1.playerData({ name: px.name || 'player', p: p || {}, ui: ui, keypress: px.keypress, bg: (assets && assets.bg && assets[ui.bg?.value]) || null, rigidBody: px.rigidBody });
...
/* SECTION: Code Editor Diff and Highlight Overlay Management */
    ...
    function syncControlsFromEditor() {
        ...
        if (ui.pHitboxRigidBody) ui.pHitboxRigidBody.checked = /hitboxMode\s*:\s*['"]rigidBody['"]/i.test(block);
    ...
    if (ui.pHitboxRigidBody) ui.pHitboxRigidBody.addEventListener('change', () => { state.lastEdited = 'player'; rerunPlayer(); });
