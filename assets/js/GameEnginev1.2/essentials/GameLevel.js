...
class GameLevel {
  ...
  constructor(gameControl) {
    ...
    // Bind once and reuse the same reference for add/removeEventListener.
    // resize.bind(this) creates a brand-new function on every call, so a
    // "matching" removeEventListener in destroy() was never actually
    // removing the listener — each level transition left its old resize
    // listener (and the level's already-destroyed gameEnv) attached to window.
    //
    // The browser fires many "resize" events per second while a window is
    // being dragged. Calling resize() on every single one repositions every
    // game object that many times per second too, which reads as jittery
    // stutter during a drag. Coalescing to at most once per rendered frame
    // (via requestAnimationFrame) keeps the canvas responsive to size
    // changes without redoing the work more often than the screen can show it.
  ...
  destroy() {
    // The resize handler's requestAnimationFrame callback can still be
    // queued when destroy() runs (e.g. a resize mid-level-transition). Mark
    // destroyed so that pending callback becomes a no-op instead of touching
    // a torn-down gameEnv/canvas.
  ...
  update() {
    // Advance the camera (if a level opted into scrolling) toward whatever
    // target the followed object set last frame — Player.update() calls
    // camera.setTarget() on itself each frame it runs. Doing this before the
    // draw pass below means every object's render this frame uses a
    // consistent camera position. No-op when gameEnv.camera is null (every
    // existing level), so this doesn't touch current rendering at all.
    if (this.gameEnv.camera) {
      this.gameEnv.camera.follow()
      this.gameEnv.camera.clampToBounds(this.gameEnv)
    }
    ...
    // Enemy/Shark/Goldfish/Pufferfish signal "player died, restart the level"
    // by setting gameEnv.gameControl.currentLevel.restart (they only hold a
    // gameEnv reference, and currentLevel is this GameLevel instance). That
    // flag was never read anywhere, so a "death" silently did nothing.
    // Re-run the current level fresh via the existing transition path.
    if (this.restart) {
      this.restart = false
