...
export class BackgroundParallax extends Background {
    ...
    constructor(data = null, gameEnv = null) {
        ...
        // Camera-relative depth scrolling (only takes effect when gameEnv.camera
        // is set — no camera means no consumer of this, unchanged behavior).
        // 0 = fixed to the screen like a distant sky; 1 = moves 1:1 with the
        // world like a foreground layer; values between give a depth effect.
        this.parallaxFactor = data.parallaxFactor ?? 0;
    ...
    draw() {
        ...
        // Camera-relative depth offset, on top of this layer's own self-scroll
        // velocity above. No camera set (every existing level) => 0 offset,
        // identical output to before this feature existed.
        const cam = this.gameEnv?.camera;
        const camOffsetX = cam ? cam.getRenderPosition().x * this.parallaxFactor : 0;
        const camOffsetY = cam ? cam.getRenderPosition().y * this.parallaxFactor : 0;

        // Calculate the wrapped position, Scrolling
        let xWrapped = (this.position.x - camOffsetX) % drawWidth;
        let yWrapped = (this.position.y - camOffsetY) % drawHeight;
