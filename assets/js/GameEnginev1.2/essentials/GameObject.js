...
class GameObject {
    ...
    /**
     * Bounding rectangle in game/container space (not viewport space).
     * Every object's canvas is positioned relative to the same shared
     * gameEnv.container via position.x/position.y (+ gameEnv.top for y), so
     * this reproduces what canvas.getBoundingClientRect() would measure —
     * without forcing a synchronous layout reflow.
     */
    getGameRect() {
        const left = this.position?.x ?? 0;
        const top = (this.gameEnv?.top || 0) + (this.position?.y ?? 0);
        const width = this.width || this.canvas?.width || 0;
        const height = this.height || this.canvas?.height || 0;
     ...
     */
    isCollision(other) {
        // Bounding rectangles computed from game state rather than
        // canvas.getBoundingClientRect(). Every object's style.left/top is
        // rewritten every frame (setupCanvas()), so calling
        // getBoundingClientRect() on every pair, every frame, was forcing a
        // synchronous layout reflow across the whole document — the classic
        // cause of stutter/jank once more than a couple of objects collide.
        const thisRect = this.getGameRect();
        const otherRect = other.getGameRect();
