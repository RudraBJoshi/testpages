...
import { resolvePhysicsProfile } from './PhysicsProfiles.js';
import { computeRigidBodyHitbox } from './HitboxAnalyzer.js';
...
class Character extends GameObject {
    ...
    constructor(data = null, gameEnv = null) {
        ...
        // 'standard' (default): use the hitbox above, hand-tuned or default.
        // 'rigidBody': once the sprite image loads, this.hitbox is replaced
        // with values derived from the sprite's actual pixels (see
        // HitboxAnalyzer.js) instead of a manually-picked percentage.
        this.hitboxMode = data?.hitboxMode === 'rigidBody' ? 'rigidBody' : 'standard';
        ...
        if (data && data.src) {
            ...
            this.spriteSheet.onload = () => {
                ...
                try {
                    ...
                    // Rigid-body mode: derive a tighter hitbox from the
                    // sprite's actual pixels instead of the hand-picked
                    // percentage passed in (or the default). Done once here,
                    // not per-frame — it's a one-time analysis of a single
                    // representative frame, not a per-render cost.
                    if (this.hitboxMode === 'rigidBody') {
                        this.hitbox = computeRigidBodyHitbox(this.spriteSheet, this.spriteData);
                    }
                } catch (err) {
        ...
        // Opt-in physics: only created when data.physics is supplied, so
        // objects that don't ask for it behave exactly as before.
        this.physicsBody = data?.physics
            ? new PhysicsBody(this, resolvePhysicsProfile(data.physics))
            : null;
   ...
   /**
    ...
    drawSprite() {
        ...
        // either property resets the whole 2D context (transform, filter, etc.)
        // even when the value is unchanged, which is wasted work every frame.
     ...
     */
    setupCanvas() {
        // When gameEnv.camera is set (a level opted into scrolling), render
        // at the camera-projected screen position instead of the raw world
        // position. With no camera (every existing level), worldToScreen is
        // never called and screen === this.position exactly as before.
        const cam = this.gameEnv.camera;
        const screen = cam ? cam.worldToScreen(this.position.x, this.position.y, this.gameEnv) : this.position;

        this.canvas.style.width = `${this.width}px`;
        this.canvas.style.height = `${this.height}px`;
        this.canvas.style.position = 'absolute';
        this.canvas.style.left = `${screen.x}px`;
        this.canvas.style.top = `${this.gameEnv.top + screen.y}px`;
    ...
    move(x, y) {
        ...
        // Physics-enabled objects are fully owned by PhysicsBody (gravity,
        // friction, world-bounds response) instead of the manual clamp code
        // below, which stays exactly as-is for every object that didn't opt
        // into data.physics.
        if (this.physicsBody) {
            this.physicsBody.step();
            return;
        }
    ...
    resize() {
        ...
        // Physics-enabled objects live in world space, which can be far
        // larger than the viewport (a scrolling level) — rescaling position
        // proportionally to viewport size, and clamping it to viewport
        // bounds, would corrupt world position the moment it exceeds one
        // viewport width/height. PhysicsBody already clamps against
        // gameEnv.levelBounds every frame, so skip both here and only
        // recompute the viewport-relative rendering/speed scale factors below.
        if (!this.physicsBody) {
