...
class Barrier extends GameObject {
    constructor(data, gameEnv) {
        ...
        // Store the raw data so collision-reaction code elsewhere (e.g.
        // Player.handleCollisionReaction's hazard check) can read metadata
        // like data.hazard through the same `spriteData` convention every
        // other collidable object already uses.
        this.data = data;
    ...
    setupCanvas() {
        ...
        // Camera-aware the same way Character.setupCanvas() is: with no
        // gameEnv.camera set (every existing level), screen === this.x/this.y
        // exactly as before.
        const cam = this.gameEnv?.camera;
        const screen = cam ? cam.worldToScreen(this.x, this.y, this.gameEnv) : { x: this.x, y: this.y };

        this.canvas.style.width = `${this.width}px`;
        this.canvas.style.height = `${this.height}px`;
        this.canvas.style.left = `${screen.x}px`;
        const topOffset = (this.gameEnv?.top || 0);
        this.canvas.style.top = `${topOffset + screen.y}px`;
    }
    ...
    resize() {
        ...
        // World-space levels (gameEnv.levelBounds set) keep barriers at fixed
        // world coordinates on resize — the camera transform in setupCanvas()
        // already handles mapping world position to the new viewport size, so
        // rescaling x/y/width/height here as well would double-apply the
        // change (and relative-position/-size barriers would drift from
        // their intended world placement).
        if (this.gameEnv.levelBounds) {
            this.canvas.width = Math.max(1, this.width);
            this.canvas.height = Math.max(1, this.height);
            this.update();
            return;
