...
export class Background extends GameObject {
    ...
    constructor(data = null, gameEnv = null) {
        ...
        this.isDestroyed = false; // Guards against onload firing after destroy()

        // Finish initializing the background after the image loads
        this.image.onload = () => {
            // A fast level transition can destroy this object before its image
            // finishes loading. Without this guard the callback still fires and
            // appends a canvas from the old, already-torn-down level into the
            // new level's container — a ghost background flashing in.
        ...
        this.image.onerror = () => {
            console.warn(`Background: failed to load image "${data.src}"`);
        };
    }
